...
We can establish isomorphisms between the Lie algebra $\mathfrak{g}$ and tangent space at any other point $p\in \mathcal{M}$ using [differentials](https://en.wikipedia.org/wiki/Pushforward_(differential)) of either left $L_p(q) = p \circ q$ or right $R_p(q) = q \circ p$ translations.
