...
For differential geometry from a computational perspective see~\cite{GallierQuaintance:2020}.
...
\subsubsection{Tangent spaces} Consider any \( p \in \mathcal{M} \) on a smooth manifold.
In order to take (directional) derivatives at the point \( p \), consider the set of all smooth curves \( c\colon (-\epsilon, \epsilon) \to \mathcal{M} \) with \( c(0) = p \) that “run through \( p \)”.
To be precise, smoothness of the curve \( c \) means that for a chart \( \varphi \) with \( p \in \mathcal{U} \), the map \( c_\varphi(t) = \varphi_i(c(t)) \) is a smooth in the classical sense as a map from \( (-\epsilon, \epsilon) \) to \( \mathbb{R}^n \). Since transitions of charts are smooth, this is independent of the choice of the actual chart.
Hence we can now consider the derivative \( \dot c_\varphi(0)  \in \mathbb{R}^n \) of the curve in the chart \( \varphi_i \). This can again be made independent of the chart and we obtain \(\dot c(0)\).

In a single chart, any derivative \(\dot c_\varphi(0)\) is equivalent to the first order to the curve \(t \to \varphi^{-1}(\varphi(p) + bt)\) for some \(b \in \mathbb{R}^n\). The vector \(b\) contains \term{coordinates} of the tangent vector in a basis induced by \(\varphi\).
Identifying each derivative \(\dot c(0)\) with its coordinates \(b\) in a chart \(\varphi\) thus results in a one-to-one correspondence between curves through \( p \) and vectors in \( \mathbb{R}^n \), formally this identification yields equivalence classes of curves.
Overall we obtain the \term{tangent space} \( T_p \mathcal{M} \) at \( p \in \mathcal{M} \), which is a vector space of dimension \( n \).
Informally speaking, the tangent space contains all directions in which one can tangentially pass through the point \( p \) on the manifold \( \mathcal{M} \). This is (again) independent of the choice of chart and we denote a tangent vector at \( p \) by \( X \in T_p \mathcal{M} \).
...
As an example, on the sphere \( \mathbb{S}^n \) the tangent space at a point \( p \in \mathbb{S}^n \) is given by
\begin{equation*}
...
On Euclidean space, the one chart \( \varphi(p) = p \) is enough to cover the whole space and we obtain that all tangent spaces are isomorphic to \( \mathbb{R}^n \) itself.

The set of operations available on a smooth manifold is very limited, so in practice one has to extend it to be able to analyze some manifold-valued data.
One of the most common ways is introducing inner product \( \langle \cdot, \cdot \rangle_p \colon T_p \mathcal{M} \times T_p \mathcal{M} \to \mathbb{R} \) on each tangent space \( T_p \mathcal{M} \).
...
\begin{itemize}
    \item geodesic, exp, log, parallel transport
    \item affine connection, there is a default for inner products.
\end{itemize}
...
Furthermore the functions \verb|copy(M, p)| and \verb|copy(M, p, X)|, as well as \verb|copyto!(M, q, p)| and \verb|copyto!(M, Y, p, X)| create copies of or copy over values of one point to another.

We also extend the \verb|rand| interface: \verb|rand(M)| generates a random point, \verb|rand(M; vector_at=p)| a random tangent vector. Both can be used with an random number generator as additional first argument. Random points and tangent vectors can also be generated in-place using \verb|rand!(M, p)| and \verb|rand!(M, X; vector_at=p)|.
...
\subsection{Functions on manifolds with an affine connection}
