...
@testset "Testing a NonDecoratorManifold - emptytrait fallbacks" begin
    ...
    @test_throws MethodError exp_fused(M, p, X, 2.0)
    @test_throws MethodError exp!(M, q, p, X)
    @test_throws MethodError exp_fused!(M, q, p, X, 2.0)
    @test_throws MethodError retract(M, p, X)
    @test_throws MethodError retract_fused(M, p, X, 2.0)
    @test_throws MethodError retract!(M, q, p, X)
    @test_throws MethodError retract_fused!(M, q, p, X, 2.0)
