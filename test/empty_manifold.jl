...
@testset "Manifold with empty implementation" begin
    ...
    @test_throws ErrorException represent_in_basis(m, [0], [0], ArbitraryOrthonormalBasis())
    @test_throws ErrorException inverse_represent_in_basis(m, [0], [0], ArbitraryOrthonormalBasis())
    @test_throws ErrorException basis(m, [0], ArbitraryOrthonormalBasis())
